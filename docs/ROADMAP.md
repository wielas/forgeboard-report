...
- **Contract decisions:**
  ...
  - The metadata decoder registry accepts only exact structured envelopes. `forge.judge.v1` is `{schema, outcome, scores}` where outcome is `pass` or `bounce` and scores contains exactly finite decimal `spec_fidelity`, `scenario_integrity`, and `architectural_conformance`; `forge.block.v1` is `{schema, reason_class}` with a nonempty exact string. A declared malformed envelope fails; an unknown schema is a typed warning.
